...
$sql = sprintf( "select id, room.name, yokozuna.name, fro, promo_year from yokozuna, room where id='%s'", $id );
...
$curr_id = pg_fetch_result( $result, 0, 1 );
$curr_name = pg_fetch_result( $result, 0, 2 );
$curr_fro = pg_fetch_result( $result, 0, 3 );
$curr_promo_year  = pg_fetch_result( $result, 0, 4 );
...
// 部屋一覧を取得するSQLの作成
$sql = "select id, room.name, yokozuna.name, fro, promo_year";
